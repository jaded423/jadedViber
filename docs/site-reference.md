...
- **Script cache-busting:** `palette.js` and `turing.js` load as `/name.js?v=N` on every content page; bump `N` in all seven pages when either file changes (GitHub Pages serves `max-age=600`).
