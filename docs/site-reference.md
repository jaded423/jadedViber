...
**Site rule (2026-09-23): no horizontal scroll, anywhere.** Wide things scale to the container (SVG diagrams, the topology map) or wrap (`pre`, inline `code` with `overflow-wrap: anywhere`); never `overflow-x: auto`. A phone viewer rotates for detail.

**Screenshots on `/work/` pages** (2026-09-23): real captures only (never a generated mockup presented as the product), in `work/img/<slug>-*.png`, ≤ 300 KB each (downscale truecolor rather than palette-quantize, which bands gradients), shown in a `<figure class="shot">` (`.shot.narrow` caps a small page at 640 px). Captures of client software come from a local copy seeded with invented records, and the page says so.

- **Snek easter egg**: click logo → scanline block transition to ASCII art → fade back.
- **Script cache-busting:** `palette.js` and `turing.js` load as `/name.js?v=N` on every content page; bump `N` in every page that loads it (`grep -l 'palette.js?v=' *.html */*.html`, nine as of 2026-09-23) when either file changes (GitHub Pages serves `max-age=600`).
