...
- [ ] **Move jadedviber.com DNS to Cloudflare → `status.jadedviber.com` for the piGate status page** (added 2026-09-10; asked from piGate — the Pi side is staged there, `piGate/TODO.md`). Why: the zone is Squarespace-managed (registrar Squarespace, `ns-cloud-c*.googledomains.com` nameservers) with no API, so Caddy on the Pi cannot do a DNS-01 cert for a custom name. Steps: (1) Cloudflare free account → add site `jadedviber.com` → it imports the current records; check against `docs/site-reference.md` § DNS (4× GitHub-Pages A `185.199.108-111.153`, `www` CNAME `jaded423.github.io`, the GitHub verification TXT); (2) Squarespace → domain → nameservers → the two Cloudflare NS (propagation ≤ a day; GitHub Pages keeps working, keep the A/CNAME records **DNS-only / grey cloud** so GitHub's own cert stays valid); (3) add `A status → 100.104.88.96` DNS-only (pi-gw1's tailnet IP — private, only tailnet members can reach it); (4) My Profile → API Tokens → Edit zone DNS template, scope **Zone:DNS:Edit on jadedviber.com only** → paste into `pi-gw1:/etc/caddy/env` (`~/.secrets` rule: file, no shell export); (5) hand back to piGate for enable + verify.
      verify: dig +short NS jadedviber.com | grep -q cloudflare && dig +short status.jadedviber.com | grep -q '^100\.' && echo DNS-OK
